 ...
 * Each page passes its city slug (used as i18n namespace key) and schema params.
 * URLs and routes remain separate for SEO.
 */
...
export async function LocationPageTemplate({
  ...
  const breadcrumbLabel = t(`${citySlug}.hero.h1`).replace(/\n/g, ' ')
      ...
      <section style={{ background: '#F2F2F2', padding: 'clamp(2rem, calc(20px + 4.167vw), 6.25rem) 0' }}>
        <div className="g-container">
          ...
          <div style={{ display: 'flex', gap: 'clamp(1rem, 2vw, 2rem)', marginTop: 'clamp(2rem, 3.5vw, 4rem)', flexWrap: 'wrap' }}>
            {buttons.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                style={{ display: 'inline-block', padding: '0.6rem 1.5rem', border: '1px solid #0D0D0D', color: '#0D0D0D', fontSize: 'var(--g-bm)', textDecoration: 'none' }}
              >
                {link.label}
              </Link>
            ))}
          ...
          <div style={{ marginTop: 'clamp(2rem, 3.5vw, 4rem)', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <div className="g-gallery-loc-row" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
              ...
              <div className="g-gallery-loc-right" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gridTemplateRows: '1fr 1fr', gap: '12px', height: 'clamp(720px, 50vw, 820px)' }}>
                <GWorkImage src="/images/work/4x4-japan-fox-tattoo-graphic.jpg" alt="Japanisches Fuchs Tattoo München — Kisha"  sizes="(max-width: 767px) 100vw, 25vw" />
                <GWorkImage src="/images/work/4x4-rabbit-tattoo-graphic.jpg"    alt="Grafik Hase Tattoo München — Kisha"         sizes="(max-width: 767px) 100vw, 25vw" />
                <GWorkImage src="/images/work/4x4-sakura-tattoo.jpg"            alt="Sakura Tattoo München — Kisha"              sizes="(max-width: 767px) 100vw, 25vw" />
                <GWorkImage src="/images/work/4x4-birds-tattoo-graphic.jpg"     alt="Grafik Vögel Tattoo München — Kisha"        sizes="(max-width: 767px) 100vw, 25vw" />
              </div>
            </div>
          ...
          <div style={{ marginTop: 'clamp(1.5rem, 2.5vw, 3rem)', display: 'flex', gap: '1.5rem', flexWrap: 'wrap', alignItems: 'center' }}>
            <span style={{ fontSize: 'var(--g-tag)', color: 'rgba(13,13,13,0.5)' }}>{t(`${citySlug}.related.label`)}</span>
            <Link href={t(`${citySlug}.related.link1_href`)} style={{ fontSize: 'var(--g-bm)', color: '#0D0D0D', textDecoration: 'none', borderBottom: '1px solid currentColor', paddingBottom: '2px' }}>
              {t(`${citySlug}.related.link1`)}
            </Link>
            <Link href={t(`${citySlug}.related.link2_href`)} style={{ fontSize: 'var(--g-bm)', color: '#0D0D0D', textDecoration: 'none', borderBottom: '1px solid currentColor', paddingBottom: '2px' }}>
              {t(`${citySlug}.related.link2`)}
            </Link>
            <Link href={t(`${citySlug}.related.prices_href`)} style={{ fontSize: 'var(--g-bm)', color: '#0D0D0D', textDecoration: 'none', borderBottom: '1px solid currentColor', paddingBottom: '2px' }}>
              {t(`${citySlug}.related.prices`)}
            </Link>
          </div>
