...
export async function generateMetadata(
  ...
  return buildMetadata({
    title: t('ottobrunn.meta.title'),
    description: t('ottobrunn.meta.description'),
...
export default async function TattooOttobrunn({
  params,
}: {
  ...
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'locations' })
  const faqItems = t.raw('ottobrunn.faq.items') as Array<{ q: string; a: string }>
  const buttons = t.raw('ottobrunn.content.buttons') as Array<{ label: string; href: string }>
      ...
      <section style={{ background: '#F2F2F2', padding: 'clamp(2rem, calc(20px + 4.167vw), 6.25rem) 0' }}>
        <div className="g-container">
          <div style={{ paddingBottom: '1.25rem', borderBottom: '2px solid #0D0D0D', marginBottom: 'clamp(1.5rem, 2.5vw, 3rem)' }}>
            <h2 style={{ fontSize: 'var(--g-l)', lineHeight: 'var(--g-lh-l)', color: '#0D0D0D' }}>{t('ottobrunn.content.heading')}</h2>
          </div>
          <div className="g-text-cols" style={{ display: 'flex', justifyContent: 'flex-end', gap: 'clamp(2rem, calc(20px + 4.167vw), 6.25rem)' }}>
            <p style={{ fontSize: 'var(--g-bm)', lineHeight: 'var(--g-lh-bm)', color: '#0D0D0D', width: 'clamp(16rem, 23.3vw, 448px)', flexShrink: 0 }}>{t('ottobrunn.content.body1')}</p>
            <p style={{ fontSize: 'var(--g-bm)', lineHeight: 'var(--g-lh-bm)', color: '#0D0D0D', width: 'clamp(16rem, 23.3vw, 448px)', flexShrink: 0 }}>{t('ottobrunn.content.body2')}</p>
          </div>
          ...
          <div style={{ marginTop: 'clamp(1.5rem, 2.5vw, 3rem)', display: 'flex', gap: '1.5rem', flexWrap: 'wrap', alignItems: 'center' }}>
            <span style={{ fontSize: 'var(--g-tag)', color: 'rgba(13,13,13,0.5)' }}>{t('ottobrunn.related.label')}</span>
            <Link href={t('ottobrunn.related.link1_href')} style={{ fontSize: 'var(--g-bm)', color: '#0D0D0D', textDecoration: 'none', borderBottom: '1px solid currentColor', paddingBottom: '2px' }}>{t('ottobrunn.related.link1')}</Link>
            <Link href={t('ottobrunn.related.link2_href')} style={{ fontSize: 'var(--g-bm)', color: '#0D0D0D', textDecoration: 'none', borderBottom: '1px solid currentColor', paddingBottom: '2px' }}>{t('ottobrunn.related.link2')}</Link>
            <Link href={t('ottobrunn.related.prices_href')} style={{ fontSize: 'var(--g-bm)', color: '#0D0D0D', textDecoration: 'none', borderBottom: '1px solid currentColor', paddingBottom: '2px' }}>{t('ottobrunn.related.prices')}</Link>
          </div>
        </div>
      </section>
